...
library(dplyr)
library(ggplot2)
# 5.1 Histogram of Average Income
hist(lviv_dataset$Avg_income,
     main = "Distribution of Average Income in Lviv Districts",
     xlab = "Average Income (UAH)",
     col = "lightblue",
     border = "white")
```
...
```r
ggplot(data = your_dataframe, aes(x = numeric_var1, y = numeric_var2)) +
  geom_point(alpha = 0.7, size = 3, colour="blue") + # alpha controls transparency
  geom_smooth("gray") +
  labs(title = "[Title: e.g., Population vs Size]",
       subtitle = "Coloured by [Category Name]",
       x = "[X Label]",
       y = "[Y Label]",
       color = "[Legend Title]") +
  theme_minimal()
```
```{r}
# 5.2 Scatter Plot: Population vs Area
ggplot(data = lviv_dataset, aes(x = Area_km2, y = Population)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth() +
  labs(title = "Relationship Between Area and Population",
       x = "Area (km²)",
       y = "Population") +
  theme_minimal()
```
...
```r
ggplot(data = your_dataframe, aes(x = category_var, y = numeric_var, fill = category_var)) +
  geom_boxplot() +
  labs(title = "Comparison of [Variable] by [Category]",
       x = "[Category Name]",
       y = "[Numeric Variable]") +
  theme_light() +
  theme(legend.position = "none") # Hides legend if x-axis already shows categories
```
```{r}
...
ggplot(data = lviv_dataset, aes(x = Tourism_level, y = Avg_income, fill = Tourism_level)) +
  geom_boxplot() +
  labs(
    title = "Average Income Across Tourism Levels in Lviv Districts",
    x = "Tourism Level",
    y = "Average Income"
  ) +
  theme_light() +
...
```r
# Example: A histogram of prices, split by city
ggplot(data = your_dataframe, aes(x = numeric_var)) +
  geom_histogram(bins = 15, fill = "coral", color = "black") +
  facet_wrap(~ category_var) + # The magic line that splits the plot
  labs(title = "Distribution of [Variable] across [Categories]",
       x = "[Variable]",
       y = "Count")
```

**Important:** you could create your own custom plot instead of facet plot.
```{r}
ggplot(data = lviv_dataset, aes(x = Population)) +
  geom_histogram(bins = 15, fill = "skyblue", color = "black") +
  facet_wrap(~ Tourism_level) +
  labs(
    title = "Population Distribution Across Tourism Levels in Lviv Districts",
    x = "Population",
    y = "Count"
  ) +
  theme_minimal()
...
```
```{r}
ggsave("population_vs_area.png", width = 8, height = 6)
```
